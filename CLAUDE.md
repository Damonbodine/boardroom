# Brownfield Agent — boardroom

**Run ID:** n976wk9yb46qdqvm8y0y616w6583rt7a
**Project ID:** jd76a0gjb9yh9d6c53dj1qvt7s83m2a0
**Repo:** /Users/damonbodine/Factory512/builds/boardroom-20260326-185038/boardroom
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Meeting Minutes Generator — on meeting detail page, from agenda items + discussion notes + motions + votes, generate formatted minutes following Roberts Rules conventions. 2) Board Packet Briefing — before a meeting, AI reads all agenda items and documents, generates 1-2 page executive briefing of key decisions needed. 3) Action Item Follow-Up Drafter — for overdue action items, generate polite follow-up message to assignee with context of what was decided and why it matters. 4) Motion Impact Analyzer — when a motion is proposed, analyze what it changes, who it affects, financial implications, alignment with bylaws. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/boardroom-20260326-185038/boardroom. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n976wk9yb46qdqvm8y0y616w6583rt7a"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n976wk9yb46qdqvm8y0y616w6583rt7a"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n976wk9yb46qdqvm8y0y616w6583rt7a","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n976wk9yb46qdqvm8y0y616w6583rt7a","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n976wk9yb46qdqvm8y0y616w6583rt7a","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/boardroom-20260326-185038/boardroom`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Meeting Minutes Generator — on meeting detail page, from agenda items + discussion notes + motions + votes, generate formatted minutes following Roberts Rules conventions. 2) Board Packet Briefing — before a meeting, AI reads all agenda items and documents, generates 1-2 page executive briefing of key decisions needed. 3) Action Item Follow-Up Drafter — for overdue action items, generate polite follow-up message to assignee with context of what was decided and why it matters. 4) Motion Impact Analyzer — when a motion is proposed, analyze what it changes, who it affects, financial implications, alignment with bylaws. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n976wk9yb46qdqvm8y0y616w6583rt7a",
  "projectName": "boardroom",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Meeting Minutes Generator — on meeting detail page, from agenda items + discussion notes + motions + votes, generate formatted minutes following Roberts Rules conventions. 2) Board Packet Briefing — before a meeting, AI reads all agenda items and documents, generates 1-2 page executive briefing of key decisions needed. 3) Action Item Follow-Up Drafter — for overdue action items, generate polite follow-up message to assignee with context of what was decided and why it matters. 4) Motion Impact Analyzer — when a motion is proposed, analyze what it changes, who it affects, financial implications, alignment with bylaws. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/boardroom-20260326-185038/boardroom`
